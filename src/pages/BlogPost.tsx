...
import { api, IS_STRAPI_CONFIGURED, type BlogArticle } from "@/lib/api";
...
const BlogPost = () => {
  ...
  const [post, setPost] = useState<BlogArticle | null | undefined>(() => {
    if (!slug) return null;
    if (!IS_STRAPI_CONFIGURED) return defaultBlogArticles.find((p) => p.slug === slug) ?? null;
    return undefined;
  });
  ...
  useEffect(() => {
    ...
    if (!IS_STRAPI_CONFIGURED) {
      setPost(defaultBlogArticles.find((p) => p.slug === slug) ?? null);
      return;
    }
    let cancelled = false;
    (async () => {
  ...
  if (IS_STRAPI_CONFIGURED && post === undefined) {
    return (
      <Layout>
        <section className="min-h-[420px] animate-pulse bg-muted" aria-busy="true" aria-label="Loading blog post" />
      </Layout>
    );
  }
  ...
  if (!post) {
    return (
      <Layout>
        <SeoHelmet layers={[]} fallbackTitle="Blog post not found — Unicare Medical, Dhaka" pathForCanonical={pathname} />
        <div className="container py-[64px] text-center">
          ...
          <p className="mt-[8px] font-body text-base text-muted-foreground">The article you requested could not be found.</p>
          <Link to="/blog" className="mt-[24px] inline-block font-heading text-sm font-semibold text-primary">
            Back to Blog
          </Link>
        </div>
      </Layout>
